...
아래 모델은 2개의 sub-layer를 가진 Encoder와 3개의 sub-layer를 가진 Decoder로 이루어져 있으며, 해당 연산을 수행할 때 input과 output의 shape(batch size, seq of length, embedding size)는 모두 동일하다.
