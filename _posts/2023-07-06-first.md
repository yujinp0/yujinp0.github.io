...
scaled dot-product attention 과정에서는 1.2에서 적용했던 방식이 그대로 적용되며, head별로 각 과정을 수행한 후에 다시 결과 값을 합쳐주는 과정을 수행한다. 마지막에 Linear 모델을 적용하여 데이터의 shape를 입력에서의 shape과 동일하게 유지시켜준다.
